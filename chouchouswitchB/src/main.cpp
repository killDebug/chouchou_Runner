/**
 * 开关 B - ESP32-C3 Super Mini
 * 按下按钮时通过 ESP-NOW 向主控发送 "B"，主控可识别是开关B 按下。
...
const char* hostname = "ESP32-C3-SwitchB";
...
static const char ACK_MSG[] = "ACK_B";
...
uint8_t espNowChannel = 0;  // 当前 WiFi/ESP-NOW 信道，状态页显示便于与主机、A 对比
...
volatile unsigned long lastAckTime = 0;  // 收到主控 ACK_B，灯环显示“上线”约 2 秒
...
// 主控发 RDY：双开关就绪，B 显示彩色流水灯
volatile bool hostReadyDisplay = false;
// 主控发 TA/TB：轮到谁按（本开关为 B）
...
void setup() {
  ...
  addLog("========================================");
  addLog("=== ESP32-C3 开关 B (ESP-NOW + 灯环) ===");
  addLog("========================================");
  ...
  Serial.println("按下按钮将发送 \"B\"（主控识别为开关B）");
  printLedHelp();
}
...
void startConfigAP() {
  ...
  WiFi.mode(WIFI_AP);
  const char* apSsid = "SwitchB-Setup";
...
void setupWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // 启动 mDNS：后续可通过 ESP32-C3-SwitchB.local 访问 / OTA
...
/** mDNS 掉了自动重建（每 10s 一次），保证 OTA 的 .local 主机名可用 */
void ensureMdnsReady(unsigned long now) {
...
void handleStatusPage() {
  ...
  String page = "<!DOCTYPE html><html><head><meta charset='utf-8'><title>SwitchB 状态</title>";
  ...
  page += "<p><b>ESP-NOW</b>：" + String(espNowReady ? "已就绪" : "未就绪") + "，<b>信道</b>：" + String((int)espNowChannel) + "（当前 STA；同 WiFi 即与主机/A 一致）</p>";
  ...
  page += "<p><b>按键</b>：短按=发送 B；450ms 内双击=RST（非对战状态重置）；长按 2 秒=RST!（任意状态强制重置游戏）。</p>";
...
void setupEspNow() {
  ...
  // 显示用：当前 STA 信道（与主机、A 同 WiFi 时应一致）
  wifi_second_chan_t second = WIFI_SECOND_CHAN_NONE;
...
#define ONLINE_SHOW_MS 2000  // 收到 ACK_B 后灯环显示“上线”的时长（毫秒）
...
void updateLedTest() {
  ...
  // 主控发 RDY：双开关就绪，B 显示彩色流水灯（与伴睡效果类似）
  if (hostReadyDisplay) {
    unsigned long t = millis();
  ...
  if (!wifiConnected && ledTestMode == LED_OFF) {
    if (wifiApMode) {
      // 配网 AP 模式：蓝色闪烁，提示“请连到 SwitchB-Setup 配网”
...
void loop() {
  ...
  if (espNowReady && (now - lastHb) >= 2000) {
    ...
    if (!hostMacValid || hostStale || (hbCount % 5) == 0) {
      esp_now_send(broadcastMac, (const uint8_t*)"PB", 2);
    }
  }
